...
(function (sf) {
    ...
    function showMessage(formOptions, message, type) {
        var container, msg, close, parent, msgOpts = formOptions.messagesOptions;

        container = document.createElement(msgOpts.message.container.tag);
        container.className = msgOpts.message.container.class + ' ' + type;

        msg = document.createElement(msgOpts.message.msg.tag);
        msg.className = msgOpts.message.msg.class;
        msg.innerHTML = message;

        close = document.createElement(msgOpts.message.closeBtn.tag);
        close.className = msgOpts.message.closeBtn.class;
        //close.setAttribute("type", "button");
        close.innerHTML = msgOpts.message.closeBtn.innerHtml;
    ...
    function showMessages(formOptions, messages, type) {
        var notFound = sf.modules.helpers.tools.iterateInputs(formOptions.context, messages, function (el, msg) {
            var group = sf.modules.helpers.domTools.closest(el, formOptions.messagesOptions.messages.selector);
            if (!group) return;
            group.classList.add(type);

            var msgEl = document.createElement(formOptions.messagesOptions.messages.tag);
            msgEl.className = formOptions.messagesOptions.messages.class;
            msgEl.innerHTML = msg;
